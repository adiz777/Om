...
# Tool Name: OM
# Description: A minimalist reconnaissance script for Kali Linux.
# Created by Adiz777
...
echo \-e "\\e\[1;31mThis script requires root privileges\. Please run with sudo\.\\e\[0m"
exit 1
fi
\}
...
level\=</span>{level:-$default_level} # Use default if no input is provided
  echo "<span class="math-inline">level"
\}
function run\_scans\(\) \{
echo \-e "\\e\[1;34mRunning reconnaissance scans\.\.\.\\e\[0m"
\# Get the desired reconnaissance level once
recon\_level\=</span>(get_recon_level)
  ...
  case $recon_level in
    ...
    medium) angryip scanner --range "$target"/24 -d 100 -o "om_results/angryip/$target/angryip_scan.txt" ;; # Increased delay
    high) angryip scanner --range "$target"/24 -d 10 -o "om_results/angryip/$target/angryip_scan.txt" ;; # Decreased delay for faster scan
  esac
  ...
  case $recon_level in
    ...
    medium) masscan -p1-1000 "$target" -oG "om_results/masscan/$target/masscan_scan.txt" ;; # Scan more ports
    high) masscan -p1-65535 "$target" -oG "om_results/masscan/$target/masscan_scan.txt" ;; # Scan all ports
  esac
  ...
  case $recon_level in
    ...
    medium) dnsrecon -d "$target" -t std,srv,axfr -o "om_results/dnsrecon/$target/dnsrecon_scan.xml" ;; # More record types
    high) dnsrecon -d "$target" -t std,srv,axfr,mx,soa,ns -o "om_results/dnsrecon/$target/dnsrecon_scan.xml" ;; # Even more record types
  esac
  ...
  case $recon_level in
    ...
    medium) fierce -dns "$target" -wide > "om_results/fierce/$target/fierce_scan.txt" ;; # Wider scan
    high) fierce -dns "$target" -wide -connect > "om_results/fierce/$target/fierce_scan.txt" ;; # Connect to discovered hosts
  esac
  ...
  case $recon_level in
    ...
    medium) nikto -h "$target" -evasion 1 -o "om_results/nikto/$target/nikto_scan.txt" ;; # Basic evasion techniques
    high) nikto -h "$target" -evasion 4 -o "om_results/nikto/$target/nikto_scan.txt" ;; # More advanced evasion
  esac
  ...
  case $recon_level in
    ...
    medium) wpscan --url "$target" --enumerate u -o "om_results/wpscan/$target/wpscan_scan.json" ;; # Enumerate users
    high) wpscan --url "$target" --enumerate u,p,t -o "om_results/wpscan/$target/wpscan_scan.json" ;; # Enumerate users, plugins, and themes
  esac
  ...
  case $recon_level in
    ...
    high) theharvester -d "$target" -l 1000 -b google,bing,linkedin > "om_results/
